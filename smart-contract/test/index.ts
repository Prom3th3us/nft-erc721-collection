...
describe(CollectionConfig.contractName, function () {
    ...
    it("Before any sale", async function () {
        ...
        await (
            await contract.mintForAddress(1, await owner.getAddress())
        ).wait();
        await (
        ...
        expect(
            await contract.balanceOf(await externalUser.getAddress())
        ).to.equal(0);
    });
            ...
            contract
                ...
                .whitelistMint(
                    ...
                    ),
                    {
                        value: getPrice(
                            SaleType.WHITELIST,
                            await (await contract.maxMintAmountPerTx())
                                .add(1)
                                .toNumber()
                        ),
                    }
        ...
        await expect(
            contract
                ...
                .whitelistMint(
                    1,
                    merkleTree.getHexProof(
                        keccak256(await holder.getAddress())
                    ),
                    {value: getPrice(SaleType.WHITELIST, 1)}
                )
        ).to.be.revertedWith("Invalid proof!");
    ...
    it("Owner only functions", async function () {
        ...
        await expect(
            contract
                .connect(externalUser)
                .setCost(utils.parseEther("0.0000001"))
        ).to.be.revertedWith("Ownable: caller is not the owner");
        await expect(
    ...
    it("Wallet of owner", async function () {
        ...
        ).deep.equal([BigNumber.from(4), BigNumber.from(5)]);
        expect(
    ...
    it("Supply checks (long)", async function () {
        ...
        const expectedTotalSupply =
            iterations * maxMintAmountPerTx + alreadyMinted;
